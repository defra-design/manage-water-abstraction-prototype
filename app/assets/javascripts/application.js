window.GOVUKPrototypeKit.documentReady(() => {
	...
	const applySearchFilterButton = document.getElementById(
		"apply-search-filter",
	);
	const clearSearchFilterButton = document.getElementById(
		"clear-search-filter",
	);
	...
	if (searchButton && searchInput && sessionDataElement && searchResultsTable) {
		...
		const getSelectedSearchFilter = () => {
			const selectedFilter = document.querySelector(
				'input[name="search-filter"]:checked',
			);
		...
		const isFilterExplicitlySelected = () => {
			return (
				document.querySelector('input[name="search-filter"]:checked') !== null
			);
		};
			...
			return licences
				...
				.filter((result) => {
					...
					return (
						...
						(licence.holder &&
							licence.holder.toLowerCase().includes(searchTerm)) ||
		...
		if (applySearchFilterButton) {
			applySearchFilterButton.addEventListener("click", () => {
				...
				const searchFilterDetails = document.getElementById(
		...
		if (clearSearchFilterButton) {
			clearSearchFilterButton.addEventListener("click", () => {
				document
					.querySelectorAll('input[name="search-filter"]')
					.forEach((input) => {
						input.checked = false;
					});
				searchResultsContainer.style.display = "none";
	...
	if (filterDetails && applyButton && clearButton && contactsTable) {
		...
		const applyContactsFilter = () => {
			...
			const selectedNoticeTypes = selectedFilters
				.map((filter) => filterToNoticeType[filter])
				.filter(Boolean);

			getRows().forEach((row) => {
				const communicationCell = row.cells[2];
				const communicationText = communicationCell
					? communicationCell.textContent || ""
					: "";
				const matchesSelection = selectedNoticeTypes.some((noticeType) =>
					communicationText.includes(noticeType),
				);
